...
- 扫描以任务形式在游戏主线程上按 `ScanIntervalMs` 运行，只要插件启用就会执行，**所有**显示模式（包括图标模式）皆如此。它会遍历场景图、校验蒙皮网格，并在首次发现每具尸体时校准其布局——正是这一收集过程保证了切换显示模式时覆盖层不会出现空白间隔。
- 逐帧开销极小：渲染阶段基于扫描快照绘制、刷新颜色、剔除屏幕外目标并整理绘制列表。视锥剔除在扫描侧与渲染侧各执行一次。
